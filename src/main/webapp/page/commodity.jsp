...
<head>
    ...
    <title>${mobilePhone.mobileName}-回收网 -  手机回收网 | 二手手机交易 | 笔记本回收 | 平板回收</title>
    <meta name="keywords" content="${mobilePhone.mobileName} ${mobilePhone.mobileName} ${mobilePhone.mobileName} ${mobilePhone.mobileName}">
    <meta name="description" content="${mobilePhone.mobileName} ${mobilePhone.mobileName} ${mobilePhone.mobileName} ${mobilePhone.mobileName}">
...
<body>
<link rel="stylesheet" type="text/css" href="css/alert_zhe1.css">
...
<div class="header">
    <div class="page">
        <h1 class="logo"><a href="good/home"><img src="images/new_images/logo.png" width="171" height="43" title="二手手机回收交易网"></a></h1>
...
<div class="recovery">
    ...
    <div class="page2">
        ...
        <div class="heat_new">
            <input type="hidden" name="typeid" id="typeid" value="${mobilePhone.id}"/>

            <dl>
                <dt><span>请选择一种回购方式</span><a href="mobile/getMobile/${mobilePhone.id}">重新评估</a></dt>
                <dd class="clearfix heat_on" name="1">
                    <div class="heat_yellow"> <img src="images/new_jian.png" class="new_jian">
                        ...
                        <div class="yellow_r"> <span>全网最高</span>
                            <div id="pg_price" class="digits">${recyclePrice}
                                <%--${ordersCommodity.price}--%>
                                <%--<i style="top: ${(ordersCommodity.price-ordersCommodity.price%1000)/1000*-29}px;">--%>
                                    <%--<c:forEach begin="0" end="${ordersCommodity.price/1000}" var="a" step="1">--%>
                                        <%--<em>${a}</em>--%>
                                    <%--</c:forEach>--%>
                                <%--</i>--%>
                                    <%--<i style="top: ${(ordersCommodity.price-ordersCommodity.price%100)/100%10*-29}px;">--%>
                                        <%--<c:forEach begin="0" end="${ordersCommodity.price%1000/100}" var="b" step="1">--%>
                                            <%--<em>${b}</em>--%>
                                        <%--</c:forEach>--%>
                                    <%--</i>--%>
                                    <%--<i style="top: ${(ordersCommodity.price%100-ordersCommodity.price%10)/10*-29}px;">--%>
                                        <%--<c:forEach begin="0" end="${ordersCommodity.price%100/10}" var="c" step="1">--%>
                                            <%--<em>${c}</em>--%>
                                        <%--</c:forEach>--%>
                                    <%--</i>--%>
                                    <%--<i style="top: ${ordersCommodity.price%10*-29}px;">--%>
                                        <%--<c:forEach begin="0" end="${ordersCommodity.price%10}" var="d" step="1">--%>
                                            <%--<em>${d}</em>--%>
                                        <%--</c:forEach>--%>
                                    <%--</i>--%>
                            </div>
                    ...
                    <a  class="heat_input"  href="goods/checkLogin">立即下单</a>
                    <a href="javascript:;" class ="heat_hg" onclick="addCar()">加入购物车</a> </dd>
...
//                            } else if (data == "fail") {
//                                alert("请完善用户详情");
//                                window.location.href = "orders/addInfo/1";
//                            } else if (data == "error") {
//                                alert("出现未知错误");
//                            }else if (data == "custNull") {
//                                alert("用户未登录,请先登录");
            ...
            function addCar(){
                $.post("orders/addCar",
                    {},function(data){
                        if(data=="success"){
                            alert("购物车加入成功");
                            window.location.href="orders/commodityList";
                        }else if(data=="fail"){
                            alert("购物车加入失败");
                        }else if (data == "custNull") {
                            alert("用户未登录,请先登录");
        ...
        <div class="mui-mbar">
            ...
            <div class="mui-mbar-mask">
                <div class="mui-mbar-gwc">
                    <div class="mui-mbar-wz"><em id="cart_total_items" class="cart_total_items">${num}</em>
            ...
            <div class="mui-mbar-cart">
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table-list">
                        ...
                        <tr id="item_id_">
                            <td class="table-padding">
                                <div class="mui-pic">
                                    <a href="javascript:deleteCart(id)" class="close_btn"><img src="images/close1.png" onclick="deleteCart(${cart.id})"/></a>
                                    <a href="javascript:void(0)"><img src="http://img.ihuigo.com/goods/739b3910f020832305a94a6a96eec952.jpg"  width="74" height="56"/></a></div>
                            </td>
                            <td>
                                <p><a href="javascript:void(0)">${mobilePhone.mobileName}</a></p>
                            </td>
                            <td >
                                <em>￥</em><em>${recyclePrice}</em>
                            </td>
                        </tr>
                ...
                <div class="mui-jiesuan">
                    <p>已选<em id="cart_total_items2" class="cart_total_items">${num}</em>台共计<em>￥</em></em><em id="cart_total">${sum}</em></p>
                    <a href="/goods/checkLogin"><input type="button" value="去结算" class="mui-button"></a>
                </div>
                <!--结算-->
            </div>
            <!--购物车-->
...
//            alert(cartId);
            var num = parseInt($("#cart_total_items2").text());
            var sum = parseInt($("#cart_total").text());
            var price = parseInt($("#"+id).text());
            var newSum=sum-price;
//            alert(sum);
//            alert(price);
//            alert(newSum);
            $.post("goods/deleteCart", {id:id},function (data) {
                    if (data=="success") {
                        alert("商品删除成功");
                        $("#item_id_"+id).remove();
                        $("#cart_total").text(newSum);
                        $("#cart_total_items2").text(num-1);
                        $("#cart_total_items").text(num-1);
                    } else if(data=="fail"){
                        alert("商品删除失败");
                    }else if(data=="error"){
                        alert("出现未知错误");
...
//        function carToOrder() {
//            if(parseInt($("#cart_total_items2").text())==0){
//                alert("购物车没有商品");
//                return ;
//            }
//            if(confirm("是否使用默认地址")) {
//            $.post("goods/carToOrder",
//                {}, function (data) {
//                    if (data == "success") {
//                        alert("生成订单成功");
//                        window.location.href = "goods/checkLogin";
//                    }
//                    else if (data == "fail") {
//                        alert("请完善用户详情");
//                        window.location.href = "goods/addInfo/2";
//                    }
//                    else if (data == "cartNull") {
//                        alert("购物车没有商品");
//                    }else if (data == "error") {
//                        alert("出现未知错误");
//                    }else if (data == "custNull") {
//                        alert("用户未登录,请先登录");
//                        window.location.href = "goods/login";
//                    }
//                });
//        }else {
//            window.location.href = "goods/check/{id}";
//        }
//        }
    </script>
